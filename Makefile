...
bundle:
	...
	@$(RM) -r '$(BUNDLE_DIR)'
	@mkdir -p '$(BUNDLE_DIR)'
	@for d in $$(ocamlfind query -recursive -predicates native -format '%d' ramen | sort -u | grep -v /findlib) ; do \
		echo $$d ;\
		cp -r "$$d" '$(BUNDLE_DIR)/' ;\
	done
...
src/RamenDepLibs.ml:
	...
	@echo "]" >> $@
	@echo "let objfiles = [" >> $@
	@for d in $$(ocamlfind query -recursive -predicates native -format '%+a' ramen | uniq | grep -v /findlib) ; do \
		echo $$d | cut -c $(OCAML_WHERE_LEN)- | \
		sed -e 's,^\(.*\)$$,  "\1" ;,' >> $@ ;\
	done ;
	@echo "]" >> $@
...
install-bundle: bundle
	@echo "Installing libraries bundle into $(prefix)$(lib_dir)"
