Feature: Test Ramen API from the command line.
  ...
  Scenario: Can get the version number from the API.
    ...
    Then curl must mention ""result":"v3.0.0""
  ...
  Scenario: Api would understand an float id.
    Given ramen httpd --url http://localhost:8080 --api is started
    When I wait 1 second
    And I run curl with arguments --data-binary '{"method":"version","id":12.3}' http://localhost:8080
    Then curl must mention ""id":12.3".

  Scenario: Can get the list of tables.
    Given ramen httpd --url http://localhost:8080 --api is started
    When I wait 1 second
    And I run curl with arguments --data-binary '{"method":"get-tables","id":1,"params":{"prefix":"test/rand"}}' http://localhost:8080
    Then curl must mention "random_walk".

  Scenario: Can get the list of columns.
    Given ramen httpd --url http://localhost:8080 --api is started
    When I wait 1 second
    And I run curl with arguments --data-binary '{"method":"get-columns","id":1,"params":["test/color_of_the_day"]}' http://localhost:8080
