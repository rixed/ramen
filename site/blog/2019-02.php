...
<p>In case of errors happening within a running operation though, then it's like for KSQL: one has to look at the server (supervisor) logs to learn about it. It's just a bit simpler in my view because each operation being it's owbn unxi process one can use normal unix tools like ps, top, etc, to monitor the operations.</p>
...
<p>CPU consumption is more surprising. Of course I am expecting that using accurate static types and ringbuffers to directly pass around messages to be more efficient than a message queue; but given Ramen is lacking many optimisations and Kafka has been used and optimised for so long, I was actually expecting it to be faster, instead of requiring that much more CPU.</p>

<p>What setting should be changed to make it 10 times faster? Sure we could use AVRO instead of CSV files, but Ramen also had to parse the CSV (and it is quite frankly slow to do so). Or could the delivery grantees be alleviated somehow to simplify message management a bit?</p>

<p>The main explanation might reside in the third chart: Kafka materialize all message queues on disk. And to run a simple two-operations stream "graph" such as the one of this benchmark, ksql will need at least 2 topics (one for the output of each operation). Plus the one for receiving the original CSV, plus maybe intermediary ones for the actual operations (I've seen simpler queries than this one requiring up to 5 or 6 topics). That's a lot of useless (from a single machine perspective; users of other use cases may object) message bouncing and a disk writes.</p>

<p>Message bouncing is made even worse by the fact that, as messages are untyped data blobs for Kafka, then the receiver receive all of it. In the case of our 80 columns messages, it means the operation that only makes use of 5 or 6 of them will receive the full message, leading to more moving than necessary. Shall we have batched the messages in the producer, say by the default batch size of 200, then that's those 200 messages that the first consumer would receive. In contrast, in Ramen "producers" (aka parent functions) send only the requested fields to their "consumers" (aka child functions), including in deep compound data structures since recently. I don't know how much of an issue this has been in this particular case though.</p>
