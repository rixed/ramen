...
<p>The first case is required to spread Ramen over several machine, while the former is the fastest one possible as we could go as far as directly calling the next operation as we call a local function.</p>

<p>Individual messages queue per function has been chosen as a communication mechanism as it suits all of the above with little or no adjustment.</p>
...
<p>So Ramen has both the notion of functions, that are individual operations turning a given stream of input tuples into a stream of output, and programs, which are a set of functions that are compiled, ran and stopped together as a whole. Within a program, loops are allowed and functions can be defined in any order. This is quite similar to compiling a normal program.</p>
