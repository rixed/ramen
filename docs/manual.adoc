...
It is more useful to distinguish between stateless and statefull functions,
...
will be the value of the first non-null argument.

===== String functions

`length`, prefix, +string -> uint16+.
...
`exp`, prefix, +num -> num+, exponential.

`log`, prefix, +num -> num+, logarithm.

`sqrt`, prefix, +num -> num+, square root.
...
CIDR must correspond to that.

===== Miscellaneous stateless functions

`hash`, infix, +any -> int64+, turn anything into a 64 bits integer.

==== Aggregate functions

Aggregate functions are statefull functions that combines the current value
...
instance, `lag (3, f)` returns the value of f 3 steps earlier.
...
over the past k same seasons. For instance, if `v` has a seasonality of 4,
...
in the example above as well as in the function below the current value is
...
exponent (between 0 and 1, the smaller and the softer the smoothed value will
be).

`smooth`, prefix, +num -> float+, same as above with a default smoothing
factor of 0.5.

==== Miscellaneous Statefull Functions

`remember`, prefix, TODO. Parameters are about to change. Cf.
https://github.com/PerformanceVision/ramen/issues/64
...
as they all have the same type. That is also the type of the result of the
CASE expression.
