...
[source,sql]
----
SELECT avg salary FROM person GROUP BY age, is_male
----

What happens here for each incoming tuple:

1. Extract the fields age and is_male and makes it the `key` of this tuple;

2. Look for the group for this key.

  1. If not found, create a new group made only of this tuple. Initialize its
  average salary with this person's salary;

  2. If found, add this salary to the average computation.
...
FROM person GROUP BY age, is_male
----
...
----
  SELECT avg salary, *
  FROM person GROUP BY age, is_make
----
...
what functions are used in the operation) and compute the current output-tuple.

With all this, evaluates the commit clause: if it is true, output the output
tuple. Also, should this input tuple be replayed when flushing this aggregate,
store it. If the output tuple has been committed, then also evaluates the
flush-when clause. If it is also true then flush this aggregate -- either by
deleting the aggregate altogether or by replacing it with an aggregate build
from replaying some of the stored input tuple.

Pictures might help, especially to help with what tuple can be used in which
clauses:

image::RamenOperations.png[]

image::RamenOpDetails.png[]
