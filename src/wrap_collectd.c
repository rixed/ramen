...
CAMLprim value wrap_collectd_decode(value buffer_, value nb_bytes_)
{
  ...
  for (unsigned i = 0; i < nb_metrics; i++) {
    ...
    assert(COLLECTD_NB_VALUES == 5);
    printf("%s.%s.%s.%s.%s = %f/%f/%f/%f/%f [%f]\n",
      m->host, m->plugin_name, m->plugin_instance, m->type_name, m->type_instance,
      m->values[0], m->values[1], m->values[2], m->values[3], m->values[4], m->time);
  }
