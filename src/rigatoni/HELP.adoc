...
to scale to thousands of servers, Ramen is designed to be efficient on a single
(or very few) servers.
...
The tuple that has been received as input.  Its name is `in` but that's also
...
In each of the strings, `${field_nae}` would be replaced by the actual value
