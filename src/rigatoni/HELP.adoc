...
It is only useful for its virtual fields.
...
| `selected.last`
| Contains the fields of the last tuple that passed the `where` filter. When `in` is the first tuple to pass that filter, then `selected.last` is the same as `in`.
...
| `unselected.last`
| Contains the fields of the last tuple that failed the `where` filter. When `in` is the first tuple to pass that filter, then `unselected.last` is the same as `in`.
