...
`tuple_name.field_name`. The prefix (before the dot) can be omitted and then
the field is understood to refer to the "in" tuple (the input tuple).
...
Named `selected`, this is the tuple that passed the where filter.  It is
obvious that the input tuple has passed that filter as well, otherwise the
expression wouldn't be evaluated, so what's the point?

The difference is the virtual fields of those tuples (see next section about
<<virtual-fields>> for a more thorough explanation).

The `selected` tuple can be used anywhere but in a group-by clause.

[[unselected-tuple]]
===== Unselected tuple

Named `unselected`, this is the tuple that failed the where filter.
...
A tuple that represents the current aggregate of a +GROUP BY+ operation.
It can be used only once the key is known, so only in the commit and flush-when
clauses.

[[output-tuple]]
===== Output tuple

The tuple that is going to be output. Its name is `out`.  The only places
...
on some conditions it is OK to leave out the "out" prefix as well). This would
be an equivalent, more explicit statement:
...
aggregation.  Can therefore only be used in a +GROUP BY+ operation, anywhere
but in the group-by clause itself.
...
previous value for `out`, which have been output only if the commit expression
returned true.
...
'virtual' fields, that is fields which value is computed rather than stored. To
...
| `in.last`
| Contains the fields of the previous input tuples. When `in` is the first tuple ever, then `in.last` is the same as `in`.
...
| `selected.last`
| Contains the fields of the last tuple that passed the `where` filter. When `in` is the first tuple to pass that filter, then `selected.last` is the same as `in` but can be distinguished using its `#count` virtual field.
...
| `unselected.last`
| Contains the fields of the last tuple that failed the `where` filter. When `in` is the first tuple to pass that filter, then `unselected.last` is the same as `in` but can be distinguished using its `#count` virtual field.

| `group.#count`
| In a +GROUP BY+ operation, how many tuples were added so far to form the output tuple. Can be used both in the where clause and in the select clause.

| `group.#successive`
| In a +GROUP BY+ operations, how many successive incoming tuples were assigned to that group (same `group by` key).

| `group.first`
| Contains the fields of the first incoming tuple added to that group. Can also be abbreviated into `first`.

| `group.last`
| Contains the fields of the last incoming tuple added to that group. Can also be abbreviated into `last`.

| `group.previous`
| Contains the fields of the last computed value of the tuple that _would_ be output shall the commit condition yield true.
