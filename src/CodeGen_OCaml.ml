...
let rec emit_for_serialized_fields
          indent typ copy skip force_null fm_var val_var oc out_var =
  let p fmt = emit oc indent fmt in
  if is_scalar typ.structure then (
    ...
    p "  | RamenFieldMask.ForceNull ->" ;
    force_null (indent + 2) oc (val_var, typ) ;
    ...
    let emit_for_record kts =
      ...
      Array.iteri (fun i (k, t) ->
        ...
        emit_for_serialized_fields
          (indent + 4) t copy skip force_null fm_var (item_var k) oc out_var
      ) ser ;
      p "      %s) in" out_var
    in
    match typ.structure with
    | TVec (_, t) | TList t ->
        ...
        emit_for_serialized_fields
          (indent + 1) t copy skip force_null "fm_" "x_" oc out_var ;
        p "  %s" out_var ;
        p ") %s fm_ in" out_var
    | TTuple ts ->
        let kts = Array.mapi (fun i t -> string_of_int i, t) ts in
...
let emit_serialize_tuple indent name oc typ =
  ...
  and force_null indent oc _ =
    emit oc indent "offs_, nulli_ + 1"
  in
  emit_for_serialized_fields_of_output
    (indent + 2) typ copy skip force_null "fieldmask_" oc "(offs_, nulli_)" ;
  p "  offs_\n"
...
let emit_factors_of_tuple name func oc =
  let typ = O.out_type_of_operation ~with_private:true func.FS.operation in
  let factors = O.factors_of_operation func.FS.operation in
...
let emit_operation name top_half_name func
  ...
  | Aggregate _ ->
    ...
    let in_type =
      RamenFieldMaskLib.in_type_of_operation func.FS.operation |>
      List.map (fun f ->
        RamenTuple.{
...
let emit_replay name func opc =
  let p fmt = emit opc.code 0 fmt in
  let typ =
    O.out_type_of_operation ~with_private:false func.FS.operation in
...
let emit_orc_wrapper func orc_write_func orc_read_func oc =
  ...
  let rtyp = O.out_record_of_operation ~with_private:true func.FS.operation in
...
let compile conf func obj_name params_mod dessser_mod
  ...
  let what = "function "^ N.func_color func.FS.name in
