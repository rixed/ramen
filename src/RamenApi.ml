...
let generate_alert table ft source_file alert_info =
...
let save_alert conf table column to_keep alert_info =
  ...
  and source_file = basename ^".ramen" in
  ...
  if file_check ~min_size:1 conf_fname <> FileOk then (
    ...
    C.with_rlock conf (fun programs ->
       ...
       * running programs. *)
      generate_alert table ft source_file alert_info ;
      compile_alert conf programs program_name source_file) ;
