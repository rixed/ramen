...
module DessserGen = struct
  ...
  let to_row_binary t p =
    Wire.to_row_binary (t : N.program :> string) p
  ...
  let sersize_of_row_binary t =
    Wire.sersize_of_row_binary (t : N.program :> string)
