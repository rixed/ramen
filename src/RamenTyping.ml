  ...
  (*$inject
    ...
    let test_type_single_func op_text =
      try
        ...
        let get_parent _ = raise Not_found in
        let _params, funcs = RamenProgram.parse get_parent prog_name txt in
...
let get_types conf parents funcs params =
  ...
  Enum.iter (fun func ->
    ...
    RamenOperation.iter_expr (fun e ->
      let open RamenExpr in
      let t = typ_of e in
      match t.scalar_typ, t.nullable with
      | Some s, Some n ->
          Hashtbl.replace h t.uniq_num (s, n)
      | _ ->
          !logger.error "Cannot resolve type for %a" (print true) e)) ;
  h
