...
// Notice that we use our parent as the parent of the GraphItem, meaning
// all coordinates will be relative to that parent. Easier when it moves.
...
OperationsItem::OperationsItem(OperationsItem *parent_, QBrush brush_) :
  QGraphicsItem(parent_), brush(brush_), parent(parent_), row(-1) {}
